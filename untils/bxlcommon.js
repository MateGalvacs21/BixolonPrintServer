...
function requestPrint(strPrinterName, strSubmit, _callback,currentid) {
	_callback("");
	var serverURL = getServerURL().url;
	if (connectionMode == "ws:" || connectionMode == "wss:") {
		if(wsPrint == null)
		...
		xmlHttpReq.onreadystatechange = function() {
			if (xmlHttpReq.readyState == 4 && xmlHttpReq.status == 200) {
				...
				if(ret.search("ready") >= 0 || ret.search("progress") >= 0)	{
					...
					_callback(res.Result,currentid);
				}
			}
			else if (xmlHttpReq.readyState == 4 && xmlHttpReq.status == 404) {
				_callback("No printers",currentid);
			}
			else if(xmlHttpReq.readyState == 4) {
				_callback("Cannot connect to server",currentid);
			}
		}
	}
}
